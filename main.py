...
class ImageAnnotator:
    ...
    def show_setup_screen(self):
        # Set fixed window size for setup
        ...
        # Title
        title_label = Label(
            main_frame, text="Image Annotator Pro",
            font=("Arial", 16, "bold"), fg=self.theme["secondary"]
        )
        title_label.grid(row=0, column=0, columnspan=2, pady=(0, 5))

        subtitle_label = Label(
            main_frame, text="Create high-quality dataset annotations with ease",
            font=("Arial", 9), fg=self.theme["dark"]
        )
        subtitle_label.grid(row=1, column=0, columnspan=2, pady=(0, 15))

        # --- Dataset Configuration Section ---
        config_frame = ttk.LabelFrame(main_frame, text="Dataset Configuration", padding=15)
        config_frame.grid(row=2, column=0, columnspan=2, sticky="ew")

        # Input Folder
        ttk.Label(config_frame, text="Input Images Folder:").grid(row=0, column=0, sticky="w")
        self.input_entry = ttk.Entry(config_frame, width=50)
        self.input_entry.grid(row=1, column=0, padx=(0, 10), pady=2, sticky="w")
        ttk.Button(config_frame, text="Browse", command=lambda: self.input_entry.insert(0, filedialog.askdirectory()), style="Nav.TButton").grid(row=1, column=1, sticky="e")

        # Output Folder
        ttk.Label(config_frame, text="Output Folder:").grid(row=2, column=0, sticky="w", pady=(10, 0))
        self.output_entry = ttk.Entry(config_frame, width=50)
        self.output_entry.grid(row=3, column=0, padx=(0, 10), pady=2, sticky="w")
        ttk.Button(config_frame, text="Browse", command=lambda: self.output_entry.insert(0, filedialog.askdirectory()), style="Nav.TButton").grid(row=3, column=1, sticky="e")

        # Object Labels
        ttk.Label(config_frame, text="Object Labels (comma separated):").grid(row=4, column=0, sticky="w", pady=(10, 0))
        self.label_entry = ttk.Entry(config_frame, width=50)
        self.label_entry.grid(row=5, column=0, columnspan=2, pady=2, sticky="w")
        self.label_entry.insert(0, "Person, Car, Dog, Cat")

        # --- Load Previous Project Button ---
        ttk.Button(main_frame, text="Load Previous Project", command=self.load_previous_project, style="Nav.TButton").grid(row=3, column=0, columnspan=2, pady=15)

        # --- Action Buttons (Start / Exit) ---
        ttk.Button(main_frame, text="Start New Project", command=self.proceed_to_annotation, style="Nav.TButton").grid(row=4, column=1, sticky="e", padx=5)
        ttk.Button(main_frame, text="Exit", command=self.on_close, style="Nav.TButton").grid(row=4, column=0, sticky="w", padx=5)

        # --- Version Info ---
        ttk.Label(main_frame, text="Version 2.0 | Created for dataset preparation").grid(row=5, column=0, columnspan=2, pady=10, sticky="e")
    ...
    def proceed_to_annotation(self):
        ...
        # Set current_label to the first label
    ...
    def show_annotation_screen(self):
        # Restore main window size for annotation
        self.root.geometry("1200x800")  # Or use self.root.state('zoomed') for maximized
        ...
        self.canvas.bind("<Configure>", lambda e: self.show_image(self.current))
